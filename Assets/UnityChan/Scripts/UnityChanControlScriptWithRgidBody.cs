...
// 以下、メイン処理.リジッドボディと絡めるので、FixedUpdate内で処理を行う.
	void FixedUpdate () {
		...
		if (mode) {
		///
		Quaternion camRot = OVRManager.display.GetEyePose(OVREye.Left).orientation;
		//float h = (float) OVRManager.display.GetEyePose(OVREye.Left).orientation;				// 入力デバイスの水平軸をhで定義
		float v = 0.5f;				// 入力デバイスの垂直軸をvで定義
		anim.SetFloat("Speed", v);							// Animator側で設定している"Speed"パラメタにvを渡す
		anim.SetFloat("Direction", camRot.y); 						// Animator側で設定している"Direction"パラメタにhを渡す
		...
		transform.Rotate(0, camRot.y * rotateSpeed, 0);	
		///
		}
		///
